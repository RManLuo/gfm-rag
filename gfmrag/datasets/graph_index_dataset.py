...
class GraphIndexDataset:
    ...
    Args:
        ...
        inverse_relation_feat (Literal['text', 'inversed'], optional): How to handle inverse relations.
            - 'text': Generate text embeddings for inverse relations by adding "inverse_" prefix to the relation name.
            - 'inversed': Use the negative of the relation embeddings for inverse relations following: http://arxiv.org/abs/2505.20422
    ...
    def __init__(
        ...
        inverse_relation_feat: Literal["text", "inversed"] = "text",
        **kwargs: str,
    ) -> None:
        self.root = root
        self.name = data_name
        ...
        self.fingerprint = hashlib.md5(json.dumps(cfgs).encode()).hexdigest()
    ...
    def load_qa_data(self, graph_rebuild: bool, force_reload: bool = False) -> bool:
        ...
        if len(exist_raw_qa_data) > 0:
            ...
            if force_reload or graph_rebuild or len(need_to_process_qa_data) > 0:
                logger.warning(
    ...
    def process_graph(self) -> None:
        ...
        if self.use_relation_feat:
            ...
            if self.inverse_relation_feat == "inversed":
                ...
                inversed_relation_text_attributes = relations_df.apply(
                    lambda row: self.attributes_to_text(
                        row["attributes"], name="inverse_" + row.name
                    ),
                    axis=1,
                ).to_list()
                inversed_rel_emb = text_emb_model.encode(
                    inversed_relation_text_attributes, is_query=False
                ).cpu()
                rel_emb = torch.cat([rel_emb, inversed_rel_emb], dim=0)
        else:
            rel_emb = None
