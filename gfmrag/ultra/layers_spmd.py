...
from gfmrag.utils import get_rank, get_world_size


class AllReduceFunction(Function):
    @staticmethod
    ...
    def backward(ctx, grad_output):
        dist.all_reduce(grad_output.contiguous(), op=dist.ReduceOp.SUM)
        return grad_output
...
class GeneralizedRelationalConv(MessagePassing):
    ...
    def __init__(
        ...
        assert self.aggregate_func == "sum"
    ...
    def forward(
        self, input, query, boundary, edge_index, node2idx, idx2node, edge_idx, edge_type, size, edge_weight=None
    ):
        ...
        split_input = input[:, idx2node]
        boundary = boundary[:, idx2node]
        edge_index = node2idx[edge_index[:, edge_idx]]
        size = (len(idx2node), len(idx2node))
        edge_type = edge_type[edge_idx]
        edge_weight = edge_weight[edge_idx]
    ...
    def update(self, update, input, node2idx, idx2node, num_node):
        batch_size = input.size(0)
        dim = input.size(-1)

        all_world_input = torch.zeros(batch_size, num_node, dim).to(input)
        all_world_input[:, idx2node, :] = input
        all_world_input = AllReduceFunction.apply(all_world_input)
        all_world_update = torch.zeros(batch_size, num_node, dim).to(update)
        all_world_update[:, idx2node, :] = update
        all_world_update = AllReduceFunction.apply(all_world_update)

        output_lst = []
        for i in range(0, num_node, 16384):
            output = self.linear(torch.cat([all_world_input[:, i: i + 16384], all_world_update[:, i: i + 16384]], dim=-1))
            if self.layer_norm:
